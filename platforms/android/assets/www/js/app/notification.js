	...
	localDB.transaction(function (tx) {
        tx.executeSql('SELECT * FROM USER_SESSION', [], function (tx, results) {
			mockDB.transaction(function (tx) {
		            tx.executeSql('SELECT * FROM NOTF WHERE userid = ?', [results.rows.item(0).userid], function (tx, results) {
		               ...
		               for (i = 0; i < len; i++){
		               		msg = null;
		                    msg = '<li><a href="#">';
						  msg  = msg +  results.rows.item(i).notfname;
		                  msg = msg+'</a></li>';
		       			  $("#notificationList").append(msg);
		               }      
		            }, null);
		         });
        }, null);
